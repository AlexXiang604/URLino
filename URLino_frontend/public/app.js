...
const BASE_URL = 'https://urlino-backend-dot-rice-comp-539-spring-2022.uk.r.appspot.com/service'
...
/********************************************
 * Retrieve Original URL (Fixed)
 ********************************************/

if (retrieveBtn) {
  retrieveBtn.addEventListener('click', async () => {
    if (!shortUrlInput || !retrieveResult) return;
    const fullShortUrl = shortUrlInput.value.trim();
    retrieveResult.textContent = '';
    ...
    try {
      // Extract the short code (last part of the URL)
      ...
      const token = localStorage.getItem("token") || ""; //改了这里
      const encodedShortCode = encodeURIComponent(shortCode);
      const response = await fetch(`${BASE_URL}/retrieve/${encodedShortCode}`, {
        method: 'GET',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + token
